...
**威胁模型**帮助安全与平台团队对齐：Codex 不是「又一个聊天窗口」，而是能**读代码、跑命令、调外部工具**的 Agent。本章给出起步视角，具体控制项对照 [权限矩阵](/12-reference/permission-matrix/) 与 [沙盒](/01-foundations/sandbox-and-network/)。
...
| 威胁 | 描述 | 缓解方向 |
|---|---|---|
| Prompt 注入 | 恶意 issue/网页诱导越权操作 | 输入消毒、只读 CI、[注入专题](/11-team-enterprise/security/prompt-injection/) |
| 过度权限 | token、沙盒过宽 | 最小权限、分支保护 |
| 数据外泄 | 命令/MCP 外传仓库内容 | 网络策略、DLP、审计 Hook |
| 恶意扩展 | 未审查 Plugin/MCP | [扩展风险](/11-team-enterprise/security/plugin-and-mcp-risk/) |
