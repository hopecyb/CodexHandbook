...
作用域与冲突处理见 [作用域与优先级](/06-customization/agents-md/scope-and-precedence/)。
...
项目规则用 `AGENTS.md`；**可重复、可分享的一套步骤**更适合做成 [Skill](/07-extensions-automation/skills/skills-overview/)。

## 常见错误

- 文件过长，模型只读到前半段——**把硬约束放在前 30 行**
- 写了「永远自动 push」类高风险指令，与 [权限与审批](/01-foundations/permissions-and-approvals/) 冲突
- 团队无人维护，文档与真实脚本命令不一致
