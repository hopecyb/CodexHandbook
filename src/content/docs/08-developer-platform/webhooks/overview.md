...
**Webhook** 让你在 Codex 或 Cloud 任务状态变化时，向自有服务发送 HTTP 回调，驱动工单更新、Slack 通知或内部审批台。本章是 [开发者平台](/08-developer-platform/) 的事件集成入口。
...
简单流水线可只用 [脚本与流水线](/08-developer-platform/non-interactive/scripts-and-pipelines/)；产品化多租户服务常 **SDK + Webhook** 组合。
...
- 见 [威胁模型](/11-team-enterprise/security/threat-model/) 与 [可接受使用](/11-team-enterprise/governance/acceptable-use/)
...
- [ ] 与 [结构化输出](/08-developer-platform/non-interactive/structured-output/) 字段约定一致
