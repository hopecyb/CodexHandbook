...
CI 依赖**进程退出码**判断步骤成败。本章说明 [codex exec](/08-developer-platform/non-interactive/codex-exec/) 常见语义、重试策略与幂等设计。
...
| 审查发现 P0 问题但执行成功 | 用 [结构化输出](/08-developer-platform/non-interactive/structured-output/) 的 `pass: false` + 脚本 `exit 1` |
