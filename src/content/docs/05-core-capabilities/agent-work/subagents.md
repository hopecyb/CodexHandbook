...
与 [并行 Agent](/04-product-guides/desktop-app/parallel-agents/) 的区别：子 Agent 通常是**主 Agent 主动委派**的任务单元，而非用户手动开多个窗口（产品实现可能重叠，以当前 UI 为准）。
...
由主线程（或你）决定采用哪条路径，再进入 [探索—计划—执行—验证](/09-workflows/explore-plan-execute-verify/) 的执行阶段。
...
- [多 Agent 协作](/09-workflows/multi-agent-coordination/)
