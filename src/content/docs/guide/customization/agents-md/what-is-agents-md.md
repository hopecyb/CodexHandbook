...
它不是高级架构文档，也不是给人类新员工看的完整手册，而是给 Codex 的项目协作规则。

## 一个核心概念

把它看成**新同事入职手册的浓缩版**：只写「在这个仓库里怎么干活」，不写公司发展史。
...
通常不用。  
这正是 `AGENTS.md` 的意义之一：
把**稳定不变的项目规则**放进去，避免每次开新任务都重复讲一遍。

### 2. 它是不是等于“我写了，Codex 就一定百分百照做”？

`AGENTS.md` 很重要，但它仍然要和这些东西一起工作：

- 你这次任务里的明确要求
- 产品的权限与审批机制
- 沙箱、网络和团队策略

更准确地说，它能帮助对齐，但不是万能强制器。

### 3. 如果我不是程序员，还需要关心这个文件吗？

需要了解基本概念。  
哪怕你不自己写代码，只要你在让 Codex 持续处理一个项目，`AGENTS.md` 都能帮你减少重复解释和跑偏。
...
如果你不确定某句话该写在哪里，可以用这个规则：

- **这个规则长期都成立**：更适合写进 `AGENTS.md`
- **这只是本次任务的特殊要求**：写在当前任务提示词里
- **这是一套可以反复复用的步骤**：考虑做成 Skill

比如：

- “这个仓库统一用 `pnpm test` 验证” -> 适合放 `AGENTS.md`
- “这次只改登录页，不要碰注册页” -> 适合写在当前任务里
- “每次发版前都跑同一套检查流程” -> 更适合做成 Skill
...
**核验依据：** 已按 OpenAI Developers 当前对项目上下文、任务边界与审批/沙盒约束的公开说明交叉复核；页面内容限定为 `AGENTS.md` 的用途、边界与常见写法，不包含未证实的强制优先级细节。  
**最近核验：** 2026-07-26
