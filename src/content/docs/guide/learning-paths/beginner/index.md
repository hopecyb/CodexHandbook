---
title: 初学者路径
description: 给第一次上手 Codex 的读者一条最短五步路线，从会说清任务到完成第一个小项目。
---

第一次学 Codex，最容易卡住的通常不是某一篇看不懂，而是不知道该从哪一页开始。

这一组就是给新手准备的最短路径。

## 推荐顺序

1. [与 Codex 对话](/guide/learning-paths/beginner/01-talk-to-codex/)
2. [处理文件](/guide/learning-paths/beginner/02-work-with-files/)
3. [给出清晰任务](/guide/learning-paths/beginner/03-give-clear-tasks/)
4. [审查结果](/guide/learning-paths/beginner/04-review-results/)
5. [完成第一个小项目](/guide/learning-paths/beginner/05-complete-first-project/)

## 这组适合谁

- 还没稳定跑通过一次完整任务闭环
- 知道 Codex 能做事，但不知道第一次该怎么练
- 想先学“够用的方法”，不是先学全量术语

初学者路径的目标不是学全，而是把第一次完整闭环跑通。

---

**状态：** verified  
**适用产品：** App / CLI / IDE  
**核验依据：** 已按本仓库当前初学者五步路径文件、快速上手章节与入门安全/检查相关页面交叉复核；本页仅承担“新手先按什么顺序读”的导航作用。  
**最近核验：** 2026-07-26
