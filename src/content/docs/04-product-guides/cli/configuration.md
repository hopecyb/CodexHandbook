...
CLI 配置决定**默认模型、工作目录、审批严格度、MCP 服务器**等。与 [配置基础](/06-customization/configuration/config-basics/) 概念一致，本页侧重终端用户常改项。
...
概念：[沙盒与网络](/01-foundations/sandbox-and-network/)
...
与 [连接 MCP 服务器](/07-extensions-automation/mcp/connect-an-mcp-server/) 共用配置；变更后重启会话。
...
更多：[CLI 排障](/04-product-guides/cli/troubleshooting/)
